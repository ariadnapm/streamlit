...
with col3:
    ...
    if uploaded_files and metadata_list:
        ...
        for index, file in enumerate(uploaded_files):
            ...
            file_df = pd.DataFrame({"Wavenumber": ab_x, "Absorbance": signal, "File": file.name})

            # Add the Absorbance data to the combined DataFrame
            all_data_df = pd.concat([all_data_df, file_df], ignore_index=True)
            

            

        # Create a new Excel file containing all combined data
        excelfile_combined = "combined_data" + ".xlsx"
        ...
        # Create an Excel file containing the metadata
        excelfile_metadata = "OPUS_metadata_" + datetime.now().strftime("%Y%m%d-%H%M%S") + ".xlsx"
